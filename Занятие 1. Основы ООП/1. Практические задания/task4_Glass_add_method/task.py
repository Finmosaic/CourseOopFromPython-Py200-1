...
class Glass:
    ...
    def init_occupied_volume(self, capacity_volume, occupied_volume):
    ...
    def add_water(self, volume):
        ...
        if not isinstance(volume, (int, float)):
            raise TypeError
        if volume < 0:
            raise ValueError('Нельзя долить отрицательынй объем')
        if self.occupied_volume + volume > self.capacity_volume:
            raise ValueError('Нельзя долить болше объема стакана')
    ...
    def remove_water(self, volume):
        ...
        if not isinstance(volume, (int, float)):
            raise TypeError
        if volume < 0:
            raise ValueError('Нельзя вылить отрицательынй объем')
        if self.occupied_volume - volume < 0:
            raise ValueError('Нельзя вылить больше налитого')
        self.occupied_volume -= volume
